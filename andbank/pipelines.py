...
class AndbankPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `andbank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM andbank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO andbank (date, title, link, content)"
